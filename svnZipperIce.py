...
# Get the Remove recision from the SVN
...
# Get the loca revision of the current check out repo
...
# Input:- None
# Output:- None
# Prints welcome headeer for ICE
def welcome():
	...
	print bcolors.OKBLUE +  "  $$ |  $$ |  $$\ $$ |            $$ |\$$$ |$$ |$$ |  $$ |$$ |  $$ | $$ |$$\ $$ |$$ |  $$ |            " + bcolors.ENDC
	print bcolors.OKBLUE +  "$$$$$$\ \$$$$$$  |$$$$$$$$\       $$ | \$$ |$$ |\$$$$$$$ |$$ |  $$ | \$$$$  |$$ |\$$$$$$$ |            " + bcolors.ENDC
	print bcolors.OKBLUE +  "\______| \______/ \________|      \__|  \__|\__| \____$$ |\__|  \__|  \____/ \__| \____$$ |            " + bcolors.ENDC
	print bcolors.OKBLUE +  "                                                $$\   $$ |                       $$\   $$ |            " + bcolors.ENDC
	print bcolors.OKBLUE +  "                                                \$$$$$$  |                       \$$$$$$  |            " + bcolors.ENDC
	print bcolors.OKBLUE +  "                                                 \______/                         \______/             " + bcolors.ENDC
	print bcolors.OKBLUE +  "$$$$$$$\            $$\ $$\       $$\        $$$$$$\                      $$\            $$\           " + bcolors.ENDC
	...
	print bcolors.OKBLUE +  "$$ |  $$ |$$ |  $$ |$$ |$$ |$$ |  $$ |      $$\   $$ |$$ |      $$ |      $$ |$$ |  $$ | $$ |$$\       " + bcolors.ENDC
	print bcolors.OKBLUE +  "$$$$$$$  |\$$$$$$  |$$ |$$ |\$$$$$$$ |      \$$$$$$  |\$$$$$$$\ $$ |      $$ |$$$$$$$  | \$$$$  |      " + bcolors.ENDC
	print bcolors.OKBLUE +  "\_______/  \______/ \__|\__| \_______|       \______/  \_______|\__|      \__|$$  ____/   \____/       " + bcolors.ENDC
	print bcolors.OKBLUE +  "                                                                              $$ |                     " + bcolors.ENDC
	print bcolors.OKBLUE +  "                                                                              $$ |                     " + bcolors.ENDC
	print bcolors.OKBLUE +  "                                                                              \__|                     " + bcolors.ENDC
	print bcolors.OKBLUE +  "" + bcolors.ENDC
...
def mainMenu(device):
	...
	welcome()
	if device == "m8":
		menuItems = ["Build "+bcolors.OKBLUE+"HTC One M8"+bcolors.ENDC+" Nightly zip", "Exit"]
	elif device == "m9":
		menuItems = ["Build "+bcolors.OKBLUE+"HTC One M9"+bcolors.ENDC+" Nightly zip", "Exit"]
	elif device == "htc10":
		menuItems = ["Build "+bcolors.OKBLUE+"HTC 10"+bcolors.ENDC+" Nightly zip", "Exit"]
	else:
		menuItems = ["Build "+bcolors.OKBLUE+"HTC 10"+bcolors.ENDC+" Nightly zip", "Build "+bcolors.OKBLUE+"HTC One M9"+bcolors.ENDC+" Nightly zip", "Build "+bcolors.OKBLUE+"HTC One M8"+bcolors.ENDC+" Nightly zip", "Exit"]
...
def buildZip(src, dst):
	...
	for dirname, subdirs, files in os.walk(src):
		...
		for filename in files:
			if not ((dirname == src) and (filename.endswith( ('.bat','.exe', 'zip', '.cfg', '.zip.md5', '.md5', 'linuxSvnZipper', 'svnZipper', '.txt', 'BuildICE', 'BuildICE.bat', 'macSvnZipper') ))):
	...
	for dirname, subdirs, files in os.walk(src):
		...
		for filename in files:
			if not ((dirname == src) and (filename.endswith( ('.bat','.exe', 'zip', '.cfg', '.zip.md5', '.md5', 'linuxSvnZipper', 'svnZipper', '.txt', 'BuildICE', 'BuildICE.bat', 'macSvnZipper') ))):
				...
				sys.stdout.write("%sPROGRESS: %d%%   \r%s"% (bcolors.OKBLUE, percent, bcolors.ENDC))
				sys.stdout.flush()
...
def getRemoteFileList(svnID, url):
	...
	while async_result.ready() == False:
		sys.stdout.write("%sPROCESS: Reading %s   \r%s"% (bcolors.OKBLUE, url, bcolors.ENDC))
		time.sleep(1)
		sys.stdout.write("%sPROCESS: Reading %s.   \r%s"% (bcolors.OKBLUE, url, bcolors.ENDC))
		time.sleep(1)
		sys.stdout.write("%sPROCESS: Reading %s..   \r%s"% (bcolors.OKBLUE, url, bcolors.ENDC))
...
def makeMd5Sum(path, zipName):
	...
	while async_result.ready() == False:
		sys.stdout.write("%sPROCESS: Making MD5 checksum %s.zip.md5 file   \r%s"% (bcolors.OKBLUE, path, bcolors.ENDC))
		time.sleep(1)
		sys.stdout.write("%sPROCESS: Making MD5 checksum %s.zip.md5 file.   \r%s"% (bcolors.OKBLUE, path, bcolors.ENDC))
		time.sleep(1)
		sys.stdout.write("%sPROCESS: Making MD5 checksum %s.zip.md5 file..   \r%s"% (bcolors.OKBLUE, path, bcolors.ENDC))
		...
		if returnOption == "10" or returnOption == "m9" or returnOption == "m8":
			...
			if not isWorking:
				...
				while async_result.ready() == False:
					...
					sys.stdout.write("%sPROCESS: Checked out %d files   \r%s"% (bcolors.OKBLUE, curentFileCount, bcolors.ENDC))
					time.sleep(1)
					curentFileCount = abs(origonalFileCount - getFileCount(workingDest))
					sys.stdout.write("%sPROCESS: Checked out %d files.   \r%s"% (bcolors.OKBLUE, curentFileCount, bcolors.ENDC))
					time.sleep(1)
				...
				if (getRemoteRevision(svnClient, workingDest) == getLocalRevision(svnClient, workingDest)):
					...
					if forceUpdate:
						...
						while async_result.ready() == False:
							curentFileCount = abs(origonalFileCount - getFileCount(workingDest))
							sys.stdout.write("%sPROCESS: Updated %d files   \r%s"% (bcolors.OKBLUE, curentFileCount, bcolors.ENDC))
							time.sleep(1)
							curentFileCount = abs(origonalFileCount - getFileCount(workingDest))
							sys.stdout.write("%sPROCESS: Updated %d files.   \r%s"% (bcolors.OKBLUE, curentFileCount, bcolors.ENDC))
							time.sleep(1)
